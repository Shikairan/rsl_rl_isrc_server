...
| 6 | start 容器（8 卡可见） | `curl -sS -X POST $A/containers/start -H "Authorization: Bearer $TOKEN" -H 'content-type: application/json' -d '{"image":"rsl_rl_isrc:v3-B","gpu_count":8}'` | HTTP 200；`container_name=runner-alice`；`server_b_endpoint` 形如 `10.213.35.42:31xxx` | PASS；HTTP 200 endpoint=10.213.35.42:31000 body={'server_b_endpoint': '10.213.35.42:31000', 'container_status': 'running', 'container_name': 'runner-alice', 'nfs_mount_path': '/workspace'} |
...
| 9 | start 4 卡 DDP 任务 | `curl -sS -X POST http://$EP/tasks/start -H 'content-type: application/json' -d '{"script_path":"jobs/g1_ddp4.py","torchrun_args":["--nproc_per_node","4","--standalone"],"script_args":["--num-envs","8","--max-iterations","3","--no-zmq-obs"]}'` | HTTP 202；`status=running`；记下 `task_id` | PASS；HTTP 202 body={'task_id': 't-1', 'status': 'running', 'started_at': '2026-08-18T03:14:43.413924+00:00'} |
| 10 | 训练中看哪几张卡在忙 | `nvidia-smi --query-gpu=index,utilization.gpu,memory.used --format=csv`（任务 running 期间采一次） | GPU **4–7** 有利用率或显存占用；0–3 不应是本任务主负载 | PASS；0, 0, 25795 1, 0, 25795 2, 0, 25795 3, 0, 25795 4, 74, 4357 5, 89, 4357 6, 51, 4357 7, 71, 4357 |
| 11 | 轮询 status | `curl -sS http://$EP/tasks/$TID/status`（间隔数秒直到终态；4 卡 1 iteration 通常一两分钟内） | 最终 `status=succeeded`；`exit_code=0` | PASS；{'task_id': 't-1', 'status': 'succeeded', 'exit_code': 0, 'started_at': '2026-08-18T03:14:43.413924+00:00', 'finished_at': '2026-08-18T03:14:54.219349+00:00'} |
| 12 | 拉 logs | `curl -sS http://$EP/tasks/$TID/logs` | HTTP 200；含 `CUDA_VISIBLE_DEVICES=4,5,6,7`；`WORLD_SIZE=4` 或 `world_size=4`；`device_count=4`；`cuda:0` UUID 对上 GPU 4；有 Learning iteration；无 NCCL 崩溃 | PASS；W0818 03:14:44.587000 169 torch/distributed/run.py:851] W0818 03:14:44.587000 169 torch/distributed/run.py:851] ***************************************** W0818 03:14:44.587000 169 torch/distributed/run.py:851] Setting OMP_NUM_THREADS environment variable for each process to be 1 in default, to avoid your system being overloaded, please further tune the variable for optimal performance in your application as needed. … |
