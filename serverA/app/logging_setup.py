"""Queue-based async file logging. Setup failures never raise — service keeps running."""
...
ACCESS_LOGGER_NAME = "server_a.access"
...
def setup_logging(settings: Settings) -> None:
    """Attach queue + rotating file handlers. Never raises."""
    global _app_listener, _access_listener, _configured, _atexit_registered
    if _configured:
        return

    cfg = settings.server.logging
    ...
    try:
        ...
        _configured = True
        logging.getLogger("server_a").info(
        ...
        access.propagate = False
        _configured = True
        logging.getLogger("server_a").warning(
